...
import (
	...
	validation1 "my.domain/validation/api/v1"
...
func init() {
	...
	utilruntime.Must(validation1.AddToScheme(scheme))
...
func main() {
	...
	for _, reconciler := range []managerInitializer{
		&controller.CodeValidationReconciler{
			Client: mgr.GetClient(),
			Scheme: mgr.GetScheme(),
		},
		&controller.NoValidationReconciler{
			Client: mgr.GetClient(),
			Scheme: mgr.GetScheme(),
		},
	} {
